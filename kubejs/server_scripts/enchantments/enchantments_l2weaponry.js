/* 
 * l2weaponry
 * 
 */

// stealth 潜伏
ServerEvents.recipes(event => {

    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:stealth',
        level: '2',
        pedestalItems: [
            {
                item: 'l2complements:captured_wind'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 3500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:stealth',
        level: '3',
        pedestalItems: [
            {
                item: 'l2complements:captured_wind'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 5000
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:stealth',
        level: '4',
        pedestalItems: [
            {
                item: 'l2complements:captured_wind'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 6500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:stealth',
        level: '5',
        pedestalItems: [
            {
                item: 'l2complements:captured_wind'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }, {
                item: 'l2complements:hard_ice'
            }
        ],
        sourceCost: 8000
    })
})

// heavy 沉重
ServerEvents.recipes(event => {

    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy',
        level: '2',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'forge:ingots/gold'
            }, {
                item: 'forge:ingots/gold'
            }, {
                item: 'forge:ingots/gold'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 3500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy',
        level: '3',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'forge:ingots/gold'
            }, {
                item: 'forge:ingots/gold'
            }, {
                item: 'forge:ingots/gold'
            }, {
                item: 'forge:ingots/gold'
            }
        ],
        sourceCost: 5000
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy',
        level: '4',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'forge:storage_blocks/gold'
            }, {
                item: 'forge:storage_blocks/gold'
            }, {
                item: 'forge:storage_blocks/gold'
            }
        ],
        sourceCost: 6500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy',
        level: '5',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'forge:storage_blocks/gold'
            }, {
                item: 'forge:storage_blocks/gold'
            }, {
                item: 'forge:storage_blocks/netherite'
            }
        ],
        sourceCost: 8000
    })
})

// hard_shield 坚硬护盾
ServerEvents.recipes(event => {

    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:hard_shield',
        level: '2',
        pedestalItems: [
            {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 3500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:hard_shield',
        level: '3',
        pedestalItems: [
            {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:force_field'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 5000
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:hard_shield',
        level: '4',
        pedestalItems: [
            {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:force_field'
            }, {
                item: 'l2complements:force_field'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 6500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:hard_shield',
        level: '5',
        pedestalItems: [
            {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:shulkerate_ingot'
            }, {
                item: 'l2complements:eternium_ingot'
            }, {
                item: 'l2complements:force_field'
            }, {
                item: 'l2complements:force_field'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }
        ],
        sourceCost: 8000
    })
})

// heavy_shield 固若金汤
ServerEvents.recipes(event => {

    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy_shield',
        level: '2',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:ingots/netherite'
            }, {
                tag: 'forge:ingots/netherite'
            }, {
                tag: 'forge:ingots/netherite'
            }, {
                tag: 'forge:ingots/netherite'
            }
        ],
        sourceCost: 3500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy_shield',
        level: '3',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                tag: 'forge:ingots/netherite'
            }, {
                tag: 'forge:ingots/netherite'
            }, {
                tag: 'forge:ingots/netherite'
            }, {
                tag: 'forge:ingots/netherite'
            }
        ],
        sourceCost: 5000
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy_shield',
        level: '4',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                tag: 'forge:storage_blocks/netherite'
            }
        ],
        sourceCost: 6500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:heavy_shield',
        level: '5',
        pedestalItems: [
            {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'l2weaponry:reinforced_handle'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'minecraft:anvil'
            }, {
                item: 'l2complements:eternium_ingot'
            }, {
                item: 'l2complements:eternium_ingot'
            }, {
                tag: 'forge:storage_blocks/netherite'
            }
        ],
        sourceCost: 8000
    })
})

// energized_will 杀戮之心
ServerEvents.recipes(event => {

    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:energized_will',
        level: '2',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:dusts/blaze'
            }, {
                tag: 'forge:dusts/blaze'
            }, {
                tag: 'forge:dusts/blaze'
            }, {
                tag: 'forge:dusts/blaze'
            }
        ],
        sourceCost: 3500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:energized_will',
        level: '3',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:dusts/blaze'
            }, {
                tag: 'forge:dusts/blaze'
            }, {
                tag: 'forge:dusts/blaze'
            }
        ],
        sourceCost: 5000
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:energized_will',
        level: '4',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:rods/blaze'
            }, {
                tag: 'forge:rods/blaze'
            }
        ],
        sourceCost: 6500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:energized_will',
        level: '5',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:rods/blaze'
            }, {
                tag: 'forge:rods/blaze'
            }, {
                tag: 'forge:rods/blaze'
            }
        ],
        sourceCost: 8000
    })
})

// raised_spirit 杀戮之魂
ServerEvents.recipes(event => {

    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:raised_spirit',
        level: '2',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 3500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:raised_spirit',
        level: '3',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 5000
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:raised_spirit',
        level: '4',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 6500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:raised_spirit',
        level: '5',
        pedestalItems: [
            {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                item: 'l2complements:soul_flame'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }, {
                tag: 'forge:heads'
            }
        ],
        sourceCost: 8000
    })
})

// claw_shielding 格挡
ServerEvents.recipes(event => {

    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:claw_shielding',
        level: '2',
        pedestalItems: [
            {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 3500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:claw_shielding',
        level: '3',
        pedestalItems: [
            {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 5000
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:claw_shielding',
        level: '4',
        pedestalItems: [
            {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                tag: 'forge:gems/lapis'
            }, {
                tag: 'forge:gems/lapis'
            }
        ],
        sourceCost: 6500
    })
    event.custom({
        type: 'ars_nouveau:enchantment',
        enchantment: 'l2weaponry:claw_shielding',
        level: '5',
        pedestalItems: [
            {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:explosion_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:warden_bone_shard'
            }, {
                item: 'l2complements:eternium_ingot'
            }, {
                item: 'l2complements:eternium_ingot'
            }
        ],
        sourceCost: 8000
    })
})
